...
//        }
        ...
        switcher.setOnClickListener(view -> {
            if (isEnded) {
                if (!isCustom) {
                    switcher.setTextColor(Color.parseColor("#0028FF"));
                    ...
                    switcher.setTextColor(Color.parseColor("#FFFFFF"));
...
//        }
        ...
        if (currWord.equals(endWord)) {
            ...
            if (!isCustom) {
                ...
                LayoutInflater factory = LayoutInflater.from(this);
                final View nextDialogView = factory.inflate(R.layout.star_dialog_win, null);
                nextDialogView.setMinimumHeight(200);
                final AlertDialog nextDialog = new AlertDialog.Builder(this).create();
                nextDialog.setView(nextDialogView);
                nextDialogView.findViewById(R.id.yes_s).setOnClickListener(v -> {
                    customWordRow = 0;
                    isEnded = true;
                    startCustomGame();
                    nextDialog.dismiss();
                });
                nextDialogView.findViewById(R.id.no_s).setOnClickListener(v -> {
                    customWordRow = 0;
                    isEnded = true;
                    for (int i = 0; i < tvs.length; i++) {
                        imageButtons[i].setImageResource(0);
        ...
        if (current_row == 6) {
            ...
            if (!isCustom) {
                ...
                LayoutInflater factory = LayoutInflater.from(this);
                final View nextDialogView = factory.inflate(R.layout.star_dialog_lose, null);
                nextDialogView.setMinimumHeight(200);
                final AlertDialog nextDialog = new AlertDialog.Builder(this).create();
                nextDialog.setView(nextDialogView);
                nextDialogView.findViewById(R.id.yes_s).setOnClickListener(v -> {
